...
if (file_exists($manifest)) {
    try {
        $vm         = new Manifest($manifest, $web_path . "/dist/");
        $entrypoint = $vm->getEntrypoint("src/js/main.js", false);
    } catch (Exception $error) {
        debug_event('scripts.inc', 'Vite manifest error: ' . $error->getMessage(), 3);
    }
}
?>
...
<?php } elseif ($entrypoint) { ?>
    <script type="module" src="<?php echo ltrim($entrypoint['url'], "\/"); ?>" crossorigin></script>
<?php } else { ?>
